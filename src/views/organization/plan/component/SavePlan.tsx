...
const SaveEnrollment = ({ planId }: { planId?: string }) => {
  //States
  ...
  const loadEnrollment = async () => {
    setLoading(true)
    const { data } = await getEnrollmentDetail(user as UserTable, planId as string)
    if (data) {
      ...
      keys.map(key => {
        switch (key) {
          ...
          case 'policys':
            ...
            break
          case 'courses':
            setValue(key, data[key] ? (data[key]?.map((c: any) => c._id) as any) : undefined)
            break
          default:
            setValue(key, data[key])
            break
  ...
  const loadCourses = async () => {
    const { data } = await getCourses(user as UserTable, organizationId as string)
    setCourses(data)
  }

  useEffect(() => {
    if (user) {
      planId && loadEnrollment()
      if (organizationId) {
  ...
  useEffect(() => {
    ...
    setTitle(planId ? 'Edit enrollment plan' : 'Add enrollment plan')

    setBackUrl([
      getLocalizedUrl(
        planId ? `/organization/${organizationId}/detail/enrollment-plan/${planId}/view` : `/organization/${organizationId}/detail/enrollment-plan`,
        locale as Locale
      )
    ])
                          ...
                          <div className='flex flex-wrap gap-2'>
                            {(selected as string[]).map(value => (
                              <Chip
                                ...
                                size='small'
                                label={courses?.find(c => c._id === value)?.title}
                              />
                            ))}
                          </div>
                        )}
                          ...
                          <MenuItem key={course._id} value={course._id}>
                            <div className='flex justify-between w-full items-center'>
                              <div className='flex gap-4 items-center'>
                                <UserAvatar
                                  userImg={(course as Course).user?.userImg}
                                  name={(course as Course).user?.nickName || (course as Course).user?.username}
                                  hiddenName
                                />
                                <Typography> {course.title}</Typography>
                              </div>
                              <Typography>Class Capacity:{(course as Course).capacity}</Typography>
                            </div>
                          </MenuItem>
                        ))}
                      </Select>
                      <CustomizedTooltip
                        title={`If you select a course that requires enrollment, students who complete the enrollment plan will automatically enter your course. So you can only select courses whose students' joining method is automatic joining, or change your course joining method to automatic joining
...
`}
          ...
          <Card>
            <CardContent>
              ...
              <FileUpload accept='image/*' loading={uploadLoading} handleUploadFiles={handleUploadFiles} />
